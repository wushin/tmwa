...
void mob_timer(TimerData *, tick_t tick, BlockId id, unsigned char data)
{
    ...
    if (bl->bl_types.null || !bl->bl_types.npc)
        return;
...
int mob_setdelayspawn(BlockId id)
{
    ...
    if (!bl || bl->bl_types.null || !bl->bl_types.npc)
        return -1;

    md = bl->is_npc();
    nullpo_retr(-1, md);

    if (!md || !md->bl_types.npc)
        return -1;
...
int mob_spawn(BlockId id)
{
    ...
    if (!md || md->bl_types.null || !md->bl_types.npc)
        return -1;
...
int mob_can_reach(dumb_ptr<npc_data> md, dumb_ptr<block_list> bl, int range)
{
    ...
    if (!bl->bl_types.pc && !bl->bl_types.npc)
        return 0;
...
int mob_ai_sub_hard_slavemob(dumb_ptr<npc_data> md, tick_t tick)
{
    ...
    if (!mmd || !mmd->bl_types.npc || mmd->bl_id != md->master_id)
        return 0;
...
void mobskill_castend_id(TimerData *, tick_t tick, BlockId id)
{
    ...
    if (!md->bl_types.npc || md->bl_prev == nullptr)
        return;
